...
| 文件 | 操作 | 行数 |
|------|------|------|
| `app/digest/summary_prompts.py` | 修改 | +3 |
| `app/digest/summary_generator.py` | 修改 | +7/-3 |
| `app/schemas/dashboard_types.py` | 修改 | +1 |
| `app/api/dashboard.py` | 修改 | +3/-1 |
| `admin/src/views/Dashboard.vue` | 修改 | +12/-1 |
| `admin/src/views/Digest.vue` | 重写 | +162/-8 |
| `tests/test_summary_generator.py` | 修改 | +4/-3 |
| `tests/test_digest_service.py` | 修改 | +2/-2 |
| `tests/test_dashboard_api.py` | 修改 | +81 |
| `packages/openapi-client/src/gen/types.gen.ts` | 自动生成 | +1 |
| `docs/spec/user-stories.md` | 修改 | +2/-2 |
| `docs/plans/us-045-046-low-content-timeout.md` | 新增 | 计划文件 |
...
| 问题 | 解决 |
|------|------|
| Digest.vue van-tag type prop 类型不匹配 | 定义 TagType 类型别名，getStatus 返回类型标注 |
| biome format 报错 showConfirmDialog 行过长 | 拆为多行参数格式 |
| biome format 报错 getStatus 返回类型格式 | 运行 biome format --write 自动修复 |
...
| pyright | 0 errors |
| pytest | 493 passed |
| biome check | 0 errors, 37 warnings（Vue template 已知误报） |
| vue-tsc | 0 errors |
| bun run build | 通过 |

### PR 链接
https://github.com/neuer/zhixi/pull/26
